...
def _show_rounds_table(rounds: list[dict]) -> None:
    ...
    for rd in rounds:
        ...
        for index, c in enumerate(calls):
            name = c.get("name", "?")
            ev = results[index] if index < len(results) else {}
            res = ev.get("result", {})
...
def chat(
    ...
    while True:
        ...
        turn_index += 1
        messages = [
            *trim_context(history, history_window),
        ...
        try:
            ...
            turn_record.update({"status": "provider_error", "error": type(exc).__name__})
            console.print(Panel(type(exc).__name__, title="Provider error", border_style="red"))
