...
`OPENROUTER_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY` hoặc `GEMINI_API_KEY`.
Khởi động lại UI. Model có thể chọn trên sidebar hoặc đặt bằng biến tương ứng
`OPENROUTER_MODEL`, `OPENAI_MODEL`, `ANTHROPIC_MODEL`, `GEMINI_MODEL`.
...
cấu hình của OpenAI không tự áp vào OpenRouter. Không có live provider nào
được xác minh trong bản tích hợp vì người dùng hiện chưa có key.
...
được giữ: `python chat_rich.py --provider openrouter --version v3` và
`python helpdesk_cli.py chat --provider openrouter --version v3`.
...
.\.venv\Scripts\python.exe scripts/run_experiment.py --version v0
# Chạy thật baseline + preflight, ghi JSON và cập nhật version_log.csv:
.\.venv\Scripts\python.exe scripts/run_experiment.py --version v0 --provider openrouter --run
```
...
```powershell
.\.venv\Scripts\python.exe scripts/run_experiment.py --version v3 --suite group --provider openrouter --run
.\.venv\Scripts\python.exe scripts/run_experiment.py --version v3 --suite adversarial --provider openrouter --run
